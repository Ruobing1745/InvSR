...
class InvSamplerSR(BaseSampler):
    ...
    def inference(self, in_path, out_path, bs=1,lmd=0.1):
    # def inference(self, in_path, out_path, bs=1, lmd=0.5):  # 新增 lmd 参数
        '''
        Inference demo.
        Input:
        ...
        if in_path.is_dir():
            ...
            im_cond_up = util_image.imread(in_path, chn='rgb', dtype='float32')  # h x w x c
            im_cond = util_image.img2tensor(im_cond_up).cuda()                   # 1 x c x h x w

            #开环算法
            image = self.sample_func(im_cond).squeeze(0)

            
            print(im_cond.dtype,im_cond.shape)
            ...
            #'''
            #修改变换算法参数lmd
            #lmd=0并不对应开环算法(必须将闭环算法注释掉才能计算开环性能指标)
            #lmd = 0 #选择最佳lmd，使闭环相对开环的性能指标增量最大
            print(f"lmd={lmd}")
            im_sr = torch.from_numpy(image).permute(2,0,1).unsqueeze(0).cuda()
            #bicubic插值
            im_lq = torch.nn.functional.interpolate(im_sr,size=[im_cond.shape[2],im_cond.shape[3]],mode='bicubic') 
            #area插值
            #im_lq = torch.nn.functional.interpolate(im_sr,size=[im_cond.shape[2],im_cond.shape[3]],mode='area')
            im_cond = im_cond + (im_cond-im_lq)*lmd
            im_cond = torch.clip(im_cond,0,1)
            image = self.sample_func(im_cond).squeeze(0)
            #'''
